...
func main() {
	...
	// For any state
	m.Handle("/stop", "*", func(c tele.Context, state fsm.FSMContext) error {
		_ = state.Finish(c.Data() != "")
		return c.Send("finish")
	})
	// It also for any states. Because manager don't filter this handler
	bot.Handle("/state", m.HandlerAdapter(func(c tele.Context, state fsm.FSMContext) error {
		return c.Send("your state: " + state.State().String())
	}))

	m.Handle("/set", "", func(c tele.Context, state fsm.FSMContext) error {
		_ = state.Set(MyState)
		_ = state.Update("payload", time.Now().Format(time.RFC850))
		return c.Send("set state")
	})

	m.Handle(tele.OnText, MyState, func(c tele.Context, state fsm.FSMContext) error {
